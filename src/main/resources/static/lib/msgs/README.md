...
All channels and handlers exist within the context of a status bus. The bus provides factories to create channels and handlers, in addition to a scope for referencing these components by name.

```javascript
var bus = require('msgs').bus();

bus.channel('lowercase');
bus.transformer(function (status) { return status.toUpperCase(); }, { input: 'lowercase', output: 'uppercase' });
...
- receive'ing from a queue returns the full status, not just the payload
