...
/*
 * AMD common! plugin
 * This plugin will load and wait for common files.  This could be handy when
 * loading common files as part of a component or a theme.
 * Some browsers do not support the load event handler of the link element.
 * Therefore, we have to use other means to detect when a common file loads.
 ...
 * for any common resources. They'll get loaded asap, but other code won't wait
 ...
 *      require('common!myproj/component.less') // --> myproj/component.less
 *      require('common!myproj/component') // --> myproj/component.common
 *
 * When using alternative file extensions, be sure to serve the files from
 * the server with the correct mime type (text/common) or some browsers won't
 * parse them, causing an error.
 *
 * usage:
 *      require(['common!myproj/comp.common']); // load and wait for myproj/comp.common
 *      define(['common!some/folder/file'], {}); // wait for some/folder/file.common
 *      require(['common!myWidget']);
 *
 * Tested in:
 *      Firefox 3.6, 4.font-awesome, 11-16
 *      Safari 3.font-awesome.4, 3.2.1, 5.font-awesome
 *      Chrome 19
 *      Opera 11.62, 12.01
 *      IE 6-10
		...
		try {
			...
			if (sheet) {
				...
				if (!ready && rules) {
					...
					sheet.insertRule('-curl-common-test {}', 0);
					sheet.deleteRule(0);
					ready = true;
				}
			}
